...
<template>
  <div v-if="sidebarGroups.length > 0" class="flex w-full flex-col gap-y-4">
    <div v-for="(group, index) in sidebarGroups" :key="index">
      <h3 class="pl-5 font-normal lg:pl-7">{{ group.text }}</h3>
      <ul v-if="group.items" class="mt-1.5 text-neutral-500">
        <li v-for="item in group.items" :key="item.text" class="group relative">
          <a
            :href="item.link"
